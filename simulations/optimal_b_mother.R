...
type2_error <- function(b, rho=0.7, alpha=0.05, big_T=200, d=1, ncp = 5.67, 
  ...
  p4 <-  dchisq(the_cv, df = d, ncp = ncp)*the_cv*g_q*w_q*(b*big_T)^(-q) # b to be smaller 
...
plot(try_ncp, type2_error(b =0.09, ncp = try_ncp), ylab = c("Type 2 Error"))
abline(h = c(0,1), col = "red")
...
try_ncp <- seq(1, 10, length.out = 10)
for(the_ncp in try_ncp){
  try_b <- seq(0.01, 1, length.out = 200)
  
  temp_b <- optim(.015, type2_error, method = "Brent", lower = min(try_b), upper = max(try_b),ncp = the_ncp)$par
  
  plot(try_b, type2_error(try_b, ncp = the_ncp), ylab = c("Type 2 Error"), 
...
try_b <- seq(0.01, 1, length.out = 200)
plot(try_b, sun_type2_error(try_b))
abline(h = c(0,1), col = "red")
...
# both --------------------------------------------------------------------
try_b <- seq(0.01, 1, length.out = 200)
plot(try_b, sun_type2_error(try_b), type = "l", ylim = c(0,1))
lines(try_b, type2_error(try_b), col = "blue")
abline(h = c(0,1), col = "red")
